...
class SensitiveInformationFirewall {
    fun inspect(text: String): FirewallResult {
        ...
        val representationChanged = transformed.normalized != text || transformed.compact != transformed.normalized ||
        ...
        if (representationChanged && containsSensitivePattern(transformed.normalized, transformed.compact, transformed.punctuationCompacted, transformed.confusableFolded, transformed.digitFolded)) {
    ...
    private fun containsSensitivePattern(vararg candidates: String): Boolean = candidates.any { candidate -> listOf(OTP, OTP_COMPACT, UPI_PIN, UPI_PIN_COMPACT, CVV, CVV_COMPACT, PRIVATE_KEY, API_KEY, API_KEY_COMPACT, STANDALONE_API_KEY, RECOVERY_CODE, PASSWORD_CONTEXT).any { it.containsMatchIn(candidate) } }

    private fun compactCodePoints(text: String): String = buildString(text.length) {
        var index = 0
        while (index < text.length) { val codePoint = text.codePointAt(index); val type = Character.getType(codePoint); if (!Character.isWhitespace(codePoint) && type != Character.FORMAT.toInt()) appendCodePoint(codePoint); index += Character.charCount(codePoint) }
    }

    private fun compactSensitiveLabels(text: String): String = buildString(text.length) {
        var index = 0
        while (index < text.length) { val codePoint = text.codePointAt(index); val type = Character.getType(codePoint); if (!isPunctuation(type) || codePoint == ':'.code || codePoint == '='.code) appendCodePoint(codePoint); index += Character.charCount(codePoint) }
    }
    ...
    private fun isPunctuation(type: Int): Boolean = when (type) {
        ...
        Character.END_PUNCTUATION.toInt(), Character.INITIAL_QUOTE_PUNCTUATION.toInt(), Character.FINAL_QUOTE_PUNCTUATION.toInt(), Character.OTHER_PUNCTUATION.toInt() -> true
        else -> false
    }

    private fun foldCommonLatinConfusables(text: String): String = buildString(text.length) { text.forEach { char -> append(when (char) {
        '\u0391', '\u0410' -> 'A'; '\u0392', '\u0412' -> 'B'; '\u03A7', '\u0425' -> 'X'; '\u0395', '\u0415' -> 'E'; '\u0397', '\u041D' -> 'H';
        '\u0399', '\u0406' -> 'I'; '\u039A', '\u041A' -> 'K'; '\u039C', '\u041C' -> 'M'; '\u039F', '\u041E' -> 'O'; '\u03A1', '\u0420' -> 'P';
        '\u03A4', '\u0422' -> 'T'; '\u03A5', '\u04AE' -> 'Y'; '\u0396', '\u0417' -> 'Z'; '\u03B1', '\u0430' -> 'a'; '\u03B5', '\u0435' -> 'e';
        '\u03B7', '\u043D' -> 'h'; '\u03B9', '\u0456' -> 'i'; '\u03BA', '\u043A' -> 'k'; '\u03BC', '\u043C' -> 'm'; '\u03BF', '\u043E' -> 'o';
        '\u03C1', '\u0440' -> 'p'; '\u03C4', '\u0442' -> 't'; '\u03C5', '\u0443' -> 'y'; '\u03C7', '\u0445' -> 'x'; '\u0437' -> 'z'; else -> char
    }) } }

    private fun foldUnicodeDecimalDigits(text: String): String = buildString(text.length) {
        var index = 0
        while (index < text.length) { val codePoint = text.codePointAt(index); if (Character.getType(codePoint) == Character.DECIMAL_DIGIT_NUMBER.toInt()) { val digit = Character.digit(codePoint, 10); if (digit >= 0) append(('0'.code + digit).toChar()) else appendCodePoint(codePoint) } else appendCodePoint(codePoint); index += Character.charCount(codePoint) }
    }
    ...
    companion object {
        ...
        private val PRIVATE_KEY = Regex("-----BEGIN (?:[A-Z0-9 ]{0,63})?PRIVATE KEY-----[\\s\\S]*?-----END (?:[A-Z0-9 ]{0,63})?PRIVATE KEY-----")
